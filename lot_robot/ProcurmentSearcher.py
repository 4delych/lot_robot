...
class ProcurementSearcher:
    ...
    def call_llm_lot_analysis(self, combined_text: str) -> dict:
        ...
        system_prompt = (
            "Ты помощник по анализу закупок для компании ООО «АСК Гелиос».\n\n"

            "Контекст компании (важно для оценки соответствия):\n"
            "ООО «АСК Гелиос» — многопрофильная строительно-инжиниринговая компания, выполняющая полный комплекс работ: "
            "от инженерных изысканий и архитектурного проектирования до строительства зданий «под ключ», монтажа всех видов "
            "инженерных систем, отделки, поставки материалов и сдачи объектов в аренду.\n\n"

            "Компания предоставляет услуги и выполняет работы:\n"
            "— наружные сети теплоснабжения, водоснабжения и водоотведения (канализации);\n"
            "— внутренние системы отопления, водоснабжения и канализации «под ключ»;\n"
            "— проектную документацию для инженерных разделов строительства;\n"
            "— возведение и реконструкцию промышленных объектов;\n"
            "— инжиниринговые услуги: технадзор, аудит сетей, управление строительными проектами;\n"
            "— работы для промышленности, коммерческой недвижимости, жилого сектора и инфраструктуры.\n\n"

            "Ключевая особенность компании:\n"
            "«АСК Гелиос» предлагает клиентам уникальную модель «одного окна» в строительстве. Мы объединяем под своим управлением\n"
            "не только классическое строительство и монтаж, но и смежные, но критически важные направления: проектно-изыскательские работы, поставки материалов, сдачу техники в аренду и управление готовой недвижимостью. Это позволяет заказчику минимизировать количество подрядчиков и получить полностью готовый объект с инженерной инфраструктурой и отделкой.\n\n"

            "ЧТО ЯВЛЯЕТСЯ ПРОФИЛЕМ КОМПАНИИ:\n"
            "— строительство жилых и нежилых зданий (функции генподрядчика);\n"
            "— электромонтажные, санитарно-технические и строительно-монтажные работы;\n"
            "— монтаж внутренних и наружных систем отопления, водоснабжения и канализации;\n"
            "— отделочные работы;\n"
            "— оптовая и розничная торговля строительными и отделочными материалами;\n"
            "— инженерные изыскания (геодезия, геология, экология, гидрометеорология);\n"
            "— архитектурное проектирование и разработка проектной документации;\n"
            "— управление проектами, технический и авторский надзор, строительный контроль;\n"
            "— гидрометеорология и мониторинг окружающей среды;\n"
            "— техническое регулирование, стандартизация и метрологическое обеспечение;\n"
            "— комплексное создание инженерной инфраструктуры «под ключ».\n\n"

            "ЧТО НЕ ЯВЛЯЕТСЯ ПРОФИЛЕМ КОМПАНИИ:\n"
            "— электромонтажные работы и монтаж слаботочных систем (СКС, пожарная сигнализация, видеонаблюдение), "
            "если это не часть комплексного проекта;\n"
            "— разработка архитектурной части проектов или дизайн-проектов интерьеров;\n"
            "— поставки импортного оборудования под ключ (без монтажной части компании);\n"
            "— розничные услуги населению (ремонт труб в квартире, замена радиатора физлицу);\n"
            "— международная деятельность (вне РФ).\n\n"

            "Если по тексту явно видно, что предмет закупки НЕ относится к инженерным системам, "
            "СМР, проектированию или инжинирингу в профиле компании — верни строго:\n"
            "{\"subject\":\"—\",\"work_scope\":\"—\",\"work_and_submission_timelines\":\"—\","
            "\"fit_summary\":\"Не наш профиль: предмет закупки не относится к инженерно-строительным и инжиниринговым работам ООО «АСК Гелиос».\","
            ...
            "Что требуется построить/смонтировать/спроектировать/оказать по инженерной части.\n"
            "Не включай цену и НМЦК.\n\n"

            "2) work_scope — состав работ / услуг:\n"
            "Перечисли все явно указанные работы и результаты через \"; \".\n"
            "Включай изыскания, проектирование, строительные и монтажные работы, наружные/внутренние сети, отделку, "
            "инжиниринг, технадзор, контроль качества, поставки материалов (если они в составе комплексного проекта).\n"
            ...
            "4) fit_summary — оценка соответствия профилю ООО «АСК Гелиос»:\n"
            "2–5 предложений.\n"
            "Обязательно:\n"
            "— укажи, почему лот подходит / потенциально подходит / не подходит;\n"
            "— отметь релевантность к инженерным системам, промышленному строительству и инжинирингу;\n"
            ...
            "— \"Подходит\" — если лот явно связан со строительством, инженерными системами, проектированием, изысканиями или "
            "инжинирингом в профиле компании.\n"
            "— \"Неопределенно\" — если формулировки общие или недостаточно деталей, но есть признаки потенциальной релевантности.\n"
            "— \"Не подходит\" — если предмет закупки явно непрофильный.\n"
        )
    ...
    def call_llm_lot_title_filter(self, items: list[dict]) -> dict:
        ...
        system_prompt = (
            "Ты помощник по анализу закупочных лотов для строительно-инжиниринговой компании ООО «АСК Гелиос».\n\n"
            ...
            "Компания предоставляет услуги и выполняет работы, предназначенные для:\n"
            "— обеспечения объектов наружными сетями теплоснабжения, водоснабжения и водоотведения (канализации);\n"
            "— оснащения зданий внутренними системами отопления, водоснабжения и канализации «под ключ»;\n"
            "— разработки проектной документации для инженерных разделов строительства;\n"
            "— возведения и реконструкции промышленных объектов (цехов, производственных баз, складских помещений);\n"
            "— оказания инжиниринговых услуг: технический надзор, аудит существующих сетей, управление строительными проектами;\n"
            "— выполнения работ для различных секторов: промышленные предприятия, коммерческая недвижимость, жилой сектор, "
            "объекты инфраструктуры.\n\n"

            "Ключевая особенность компании:\n"
            "«АСК Гелиос» предлагает клиентам уникальную модель «одного окна» в строительстве. Мы объединяем под своим "
            "управлением не только классическое строительство и монтаж, но и смежные, но критически важные направления: "
            "проектно-изыскательские работы, поставки материалов, сдачу техники в аренду и управление готовой недвижимостью. "
            "Это позволяет заказчику минимизировать количество подрядчиков и получить полностью готовый объект с инженерной "
            "инфраструктурой и отделкой.\n\n"

            "ЧТО ЯВЛЯЕТСЯ ПРОФИЛЕМ КОМПАНИИ:\n"
            "— строительство жилых и нежилых зданий (выполнение функций генподрядчика);\n"
            "— производство электромонтажных, санитарно-технических и строительно-монтажных работ;\n"
            "— монтаж внутренних и наружных систем отопления, водоснабжения и канализации;\n"
            "— выполнение отделочных работ (штукатурка, малярные работы, устройство покрытий);\n"
            "— оптовая и розничная торговля строительными и отделочными материалами, лесоматериалами, сантехникой, "
            "лакокрасочной продукцией, скобяными изделиями;\n"
            "— инженерные изыскания для строительства (геодезия, геология, экология, гидрометеорология);\n"
            "— архитектурное проектирование и разработка проектной документации;\n"
            "— управление проектами строительства, технический и авторский надзор, строительный контроль;\n"
            "— деятельность в области гидрометеорологии и мониторинга окружающей среды (включая контроль загрязнений);\n"
            "— услуги по техническому регулированию, стандартизации и метрологическому обеспечению;\n"
            "— комплексное создание инженерной инфраструктуры объектов «под ключ».\n\n"

            "ЧТО НЕ ЯВЛЯЕТСЯ ПРОФИЛЕМ КОМПАНИИ (ЕСЛИ ЛОТ ПРО ЭТО — СТАВЬ 1):\n"
            "— электромонтажные работы и монтаж слаботочных систем (СКС, пожарная сигнализация, видеонаблюдение) — если это не "
            "требуется в рамках комплексного проекта, данные работы передаются субподрядчикам или не выполняются;\n"
            "— разработка архитектурной части проектов или дизайн-проектов интерьеров;\n"
            "— поставки импортного оборудования под ключ (компания занимается монтажом, а не дистрибуцией);\n"
            "— розничные услуги населению (ремонт труб в квартире или замена радиатора физическому лицу) — приоритет — работа "
            "с юридическими лицами и бюджетными организациями;\n"
            "— международная деятельность (вне РФ).\n\n"
            ...
            "ШКАЛА ОЦЕНКИ:\n"
            "5 — прямое попадание в профиль компании (инженерные сети, СМР, проектирование, инжиниринг);\n"
            ...
            "— если в лоте отсутствуют явные признаки строительства, монтажа инженерных систем, изысканий, проектирования или "
            "инжиниринга — не ставь выше 3;\n"
