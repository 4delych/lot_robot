...
class ProcurementSearcher:
    ...
    def call_llm_lot_analysis(self, combined_text: str) -> dict:
        ...
        system_prompt = (
            "Ты извлекаешь сведения из закупочной документации по одному лоту.\n"
            ...
            "1) goals_tasks — Цели и задачи лота:\n"
            "- 1–3 предложения: что требуется сделать (услуги/работы) и какой ожидается результат.\n"
            "- Если в тексте есть конкретные результаты/выходные материалы (отчет, заключение, проект, ТЗ, регламент, "
            "план, обучение и т.п.) — упомяни их.\n"
            "- Не включай стоимость/НМЦК.\n\n"

            "2) timelines — Сроки выполнения (ТОЛЬКО сроки исполнения предмета закупки):\n"
            "- Включай только: срок выполнения работ/оказания услуг, срок(и) этапов, срок сдачи результатов.\n"
            "- Не включай сюда: сроки оплаты, сроки подписания актов/приемки, гарантийные сроки, сроки устранения недостатков, "
            "штрафы/пени.\n"
            "- Если сроков несколько — перечисли через '; '. Если сроков нет — '—'.\n\n"

            "3) requirements — Требования к выполнению работ/услуг:\n"
            "- Включай: состав/объем работ, требования к качеству/результату, формат результатов и отчетности, "
            "условия приемки (как требование), требования к исполнителю/персоналу/опыту (если есть), требования к данным/доступам/взаимодействию.\n"
            "- Формат: 4–10 коротких пунктов в одной строке через '; ' (без нумерации).\n"
            "- Не включай: сроки оплаты.\n\n"

            "Самопроверка перед ответом:\n"
            "- Если в timelines попали слова 'оплата', 'акт', 'приемка', 'гарантия', 'устранение недостатков' — убери их из timelines.\n"
        )
