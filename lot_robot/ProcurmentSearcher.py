...
class ProcurementSearcher:
    ...
    def call_llm_lot_analysis(self, combined_text: str) -> dict:
        ...
        system_prompt = (
            "Ты помощник по анализу закупок для компании ООО «Текрон».\n\n"

            "Контекст компании (важно для оценки соответствия):\n"
            "ООО «Текрон» — производственная компания, специализирующаяся на разработке и изготовлении одноразовых "
            "защитных комбинезонов из нетканых материалов для промышленных предприятий и специальных служб.\n\n"

            "Компания занимается производством и поставками продукции, предназначенной для:\n"
            "— защиты от опасных сухих частиц, пыли и аэрозолей (включая асбест);\n"
            "— защиты от жидких веществ невысокой опасности (водные растворы кислот и щелочей до 30–40%, масла, нефтепродукты);\n"
            "— обеспечения барьерной защиты от влаги, пара и электростатических разрядов;\n"
            "— использования в химической, нефтехимической, атомной, пищевой, фармацевтической промышленности, "
            "машиностроении, сельском хозяйстве, а также в подразделениях МЧС;\n"
            "— оптовых поставок по России напрямую со склада производителя.\n\n"

            "Ключевая особенность компании:\n"
            "Продукция производится под собственным брендом TECRON. Компания не занимается перепродажей сторонних марок "
            "или смежных категорий товаров и работает в B2B-сегменте.\n\n"

            "ЧТО ЯВЛЯЕТСЯ ПРОФИЛЕМ КОМПАНИИ:\n"
            "— производство защитных одноразовых комбинезонов (TECRON PRO, TECRON Classic);\n"
            "— оптовые поставки одноразовых комбинезонов российским предприятиям;\n"
            "— поставки продукции для задач защиты 5 и 6 типов;\n"
            "— поставки для промышленных и специальных работ (в т.ч. утилизация асбеста, малярные работы, клининг);\n"
            "— консультации по подбору комбинезонов под условия эксплуатации.\n\n"

            "ЧТО НЕ ЯВЛЯЕТСЯ ПРОФИЛЕМ КОМПАНИИ:\n"
            "— многоразовая спецодежда (куртки, брюки, робы);\n"
            "— поставка оборудования, лицензий, ПО;\n"
            "— услуги монтажа, настройки, технического обслуживания;\n"
            "— консалтинг, аудит, цифровизация;\n"
            "— розничные поштучные продажи как основной формат закупки;\n"
            "— международные поставки (вне РФ).\n\n"

            "Если по тексту явно видно, что предмет закупки НЕ связан с одноразовыми защитными комбинезонами "
            "или является чистой поставкой непрофильных товаров/услуг — верни строго:\n"
            "{\"subject\":\"—\",\"work_scope\":\"—\",\"work_and_submission_timelines\":\"—\","
            "\"fit_summary\":\"Не наш профиль: предмет закупки не относится к одноразовым защитным комбинезонам бренда TECRON.\","
            ...
            "Что именно требуется поставить / произвести / отгрузить и для каких задач.\n"
            "Не включай цену и НМЦК.\n\n"

            "2) work_scope — состав поставки / требований:\n"
            "Перечисли все явно указанные позиции и требования через \"; \".\n"
            "Включай типы защиты, материалы, плотность, размеры, упаковку, количество, условия поставки, "
            "требования к сертификатам/соответствию — только если это явно есть в тексте.\n"
            "Не включай финансовые условия и оплату.\n\n"

            "3) work_and_submission_timelines — сроки поставки и сроки подачи:\n"
            "Одна строка, два блока:\n"
            "— \"Сроки работ: ...\" (для этой компании трактуй как сроки поставки/изготовления/отгрузки);\n"
            "— \"Сроки подачи: ...\".\n"
            "Включай только явно указанные сроки.\n"
            "Если сроков нет — ставь \"—\" для соответствующего блока.\n\n"

            "4) fit_summary — оценка соответствия профилю ООО «Текрон»:\n"
            "2–5 предложений.\n"
            "Обязательно:\n"
            "— укажи, почему лот подходит / потенциально подходит / не подходит;\n"
            "— отметь релевантность к одноразовым защитным комбинезонам и B2B-поставке по РФ;\n"
            ...
            "— \"Подходит\" — если лот явно связан с поставкой/производством одноразовых защитных комбинезонов.\n"
    ...
    def call_llm_lot_title_filter(self, items: list[dict]) -> dict:
        ...
        system_prompt = (
            "Ты помощник по анализу закупочных лотов для производственной компании ООО «Текрон».\n\n"

            "ОПИСАНИЕ КОМПАНИИ:\n"
            "ООО «Текрон» — производственная компания, специализирующаяся на разработке и изготовлении одноразовых защитных "
            "комбинезонов из нетканых материалов для промышленных предприятий и специальных служб.\n\n"

            "Компания занимается производством и поставками продукции, предназначенной для:\n"
            "— защиты от опасных сухих частиц, пыли и аэрозолей (включая асбест);\n"
            "— защиты от жидких веществ невысокой опасности (водные растворы кислот и щелочей до 30–40%, масла, нефтепродукты);\n"
            "— обеспечения барьерной защиты от влаги, пара и электростатических разрядов;\n"
            "— использования в различных отраслях: химической, нефтехимической, атомной, пищевой, фармацевтической промышленности, "
            "машиностроении, сельском хозяйстве, а также в подразделениях МЧС;\n"
            "— оптовых поставок по России напрямую со склада производителя.\n\n"

            "Ключевая особенность компании:\n"
            "Продукция разрабатывается и производится под собственным брендом TECRON с фокусом на соотношение цены и защитных свойств. "
            "Компания не занимается перепродажей сторонних марок или торговлей смежными категориями товаров, концентрируясь "
            "исключительно на выпуске одноразовых комбинезонов для B2B-сегмента.\n\n"

            "ЧТО ЯВЛЯЕТСЯ ПРОФИЛЕМ КОМПАНИИ:\n"
            "— производство защитных одноразовых комбинезонов (модели TECRON PRO и TECRON Classic);\n"
            "— реализация продукции оптом (от производителя) российским предприятиям;\n"
            "— обеспечение защиты 5 и 6 типов (от твердых частиц и ограниченного разбрызгивания жидкостей);\n"
            "— поставки для промышленности, спецслужб и специализированных работ (малярные, утилизация асбеста, уборка помещений);\n"
            "— консультирование заказчиков по подбору СИЗ в зависимости от условий труда.\n\n"

            "ЧТО НЕ ЯВЛЯЕТСЯ ПРОФИЛЕМ КОМПАНИИ (ЕСЛИ ЛОТ ПРО ЭТО — СТАВЬ 1):\n"
            "— производство или продажа многоразовой спецодежды (курток, брюк, роб);\n"
            "— торговля другими средствами индивидуальной защиты (перчатками, обувью, касками, респираторами);\n"
            "— поставки оборудования, лицензий или программного обеспечения;\n"
            "— услуги по монтажу, настройке, техническому обслуживанию;\n"
            "— консалтинг, аудит или проекты по цифровизации — компания занимается только производством и продажей готовой продукции.\n\n"
            ...
            "ШКАЛА ОЦЕНКИ:\n"
            "5 — прямое попадание в профиль компании с учетом производимой продукции;\n"
            ...
            "ПРАВИЛА ОЦЕНКИ:\n"
            "— если лот относится к НЕ профильным направлениям (не про одноразовые защитные комбинезоны), ставь 1 без колебаний;\n"
            "— если лот про многоразовую спецодежду, оборудование, ПО или услуги — ставь 1;\n"
